...
function updatePlot(){
    d3.selectAll("#divCheckBox .message").remove();
  ...
  anchors=[];
    ...
    if(anchors.length<=2){
        ...
        d3.select("#divCheckBox").append("text").attr("class","message").text("Please select minimum 3");
    }
    else{
...
function plottingDataPoints(data,dataBeforeNorm)
{
    ...
    data.forEach(function(record){
        ...
        for(key in record){
            ...
            if(anchor_count<num_of_col-1){
                numerator_x=numerator_x+(anchors_xaxis[anchor_count]*record[key]);
                denominator_x=denominator_x+record[key];
                numerator_y=numerator_y+(anchors_yaxis[anchor_count]*record[key]);
                denominator_y=denominator_y+record[key];
                anchor_count++;
                toolTipMessage=toolTipMessage+key+": "+beforeNormRecord[key]+"\n";
            } 
            else{
                classColor=colorMapping[record[key]];
                toolTipMessage=toolTipMessage+key+": "+beforeNormRecord[key]+"\n";
            }
        } 
        }
        ...
        console.log("x", dataPoint_x);
        console.log("numX", numerator_x);
